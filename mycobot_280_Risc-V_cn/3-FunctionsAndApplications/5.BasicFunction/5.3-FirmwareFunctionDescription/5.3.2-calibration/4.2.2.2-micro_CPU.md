...
```python
nano riscv_mycobot_test_gui.py
```
然后按下回车键，即可创建名为riscv_mycobot_test_gui的python文件。此处的rasp_mycobot_test_gui为创立的python文件的文件名，您可以在此更改为任意名称。按下回车键后会进入代码编辑页面，您可以在此界面对进行python程序代码编写。可以把拖动示教的代码拷贝至此处。(图中的文件名为demo2)
