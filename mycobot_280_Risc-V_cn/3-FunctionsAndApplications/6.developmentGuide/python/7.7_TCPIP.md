...
1. RV4B开发板和控制端需要在同一网络
2. 服务器文件（`Server_280_RISCV.py`）需要先在RV4B开发板中执行（具体操作见下方gif操作图）
...
1. 克隆我们的项目库：`git clone https://github.com/elephantrobotics/pymycobot.git`

2. 在demo文件夹中找到 [Server_280_RISCV.py](https://github.com/elephantrobotics/pymycobot/blob/main/demo/Server_280_RISCV.py) 文件

3. 请根据您的机器型号更改Server_280.py文件最后一行MyCobotServer中传递的参数。
...
4. python 运行：`python Server_280_RISCV.py`


### 案例

在 PC 电脑端运行 `Server_280_RISCV.py` 脚本：
