...
#
# Initialize a MyCobot object
# The following is the object code for the Raspberry Pi version
mc = MyCobot(PI_PORT, PI_BAUD)
...
# The MyCobot class requires two parameters to be initialized:
...
#
# Initialize a MyCobot object
# The following is the object code for creating the Raspberry Pi version
# mc = MyCobot(PI_PORT, PI_BAUD)
...
mc.set_color(0, 50, 0) time.sleep(0.7)
```
...
#
# Initialize a MyCobot object
# Create an object code for the Raspberry Pi version
# mc = MyCobot(PI_PORT, PI_BAUD)
# M5 version
mc = MyCobot('COM3', 115200)
...
# The MyCobot class requires two parameters to initialize:
#   The first is the serial port string, such as:
#       linux: "/dev/ttyUSB0"
#       windows: "COM3"
#   The second is the baud rate:
...
mc.send_angles(angles[1], 30) time.sleep(2) mc.send_angles(angles[0], 30) time.sleep(2) mc.send_angles(angles[1], 30) time.sleep(2) #Turn off the suction pump pump_off() mc.send_angles(angles[0], 40) time.sleep(1.5)
```
